...
for iSub = 1:nSub
    ...
    for iStimulus = 1:nStimPerFile
        ...
        for iBatch = 1:nBatch
            ...
            % predicted response
            if opt.unpad.do
                pred = LM_convfft(feature{iStimulus},model(:,:,:,idxReg),'valid');
            else
                pred = LM_convfft(feature{iStimulus},model(:,:,:,idxReg),'full');
            end
            if opt.removeMean
            ...
            % true response
            if opt.unpad.do
                unpad = LM_laggedDims(nx,iB(iStimulus),size(response,1),minLag,maxLag);
                assert( unpad.xb == 1 && unpad.xe == nx);
                y = response;
                iB_ = iB(iStimulus);
                n = nx;
            else
                y = LM_pady(response,nx,nPad_b,nPad_e,iB(iStimulus));
                iB_ = 1;
                n = size(y,1);
            end
