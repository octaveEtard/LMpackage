function [XtX,Xty,mX,mY,N] = backward_crossMatrices(stimOpt,EEGopt,opt)
%
% backward_crossMatrices
...
for iStimulus = 1:nStimPerFile
    ...
    if unpad
        opt.unpad = LM.laggedDims(nx(iStimulus),1,nx(iStimulus),minLag,maxLag);
        opt.unpad.do = true;
    end
    
    [feature{iStimulus},mY(:,iStimulus),N(iStimulus),...
        Ytop{iStimulus},Ybottom{iStimulus}] = LM.computeYFFT(feature{iStimulus},minLag,maxLag,nFFT,opt);
end
...
for iSub = 1:nSub
    ...
    for iStimulus = 1:nStimPerFile
        ...
        [XtX_,xF,mX(:,iStimulus,iSub),Xtop,Xbottom] = LM.laggedXtX(response((1:nx(iStimulus))+iB(iStimulus)-1,:),minLag,maxLag,opt);
        
        Xty_ = LM.laggedXty(xF,feature{iStimulus},minLag,maxLag,...
