function model = fitLinearModel(X,y,opt)
%
% fitLinearModel
...
%
% See also: LM.ridge, LM.truncatedEigenRegression
%
% ------ sanity checks
...
switch opt.method.name
    ...
    case 'trunc-eig-XtX'
        model = LM.truncatedEigenRegression(X,y,opt.accumulate,opt.method);

    case 'ridge-eig-XtX'
        model = LM.ridge(X,y,opt.accumulate,opt.method);
end
